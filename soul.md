...
# 專案架構鐵則 (委派時需監督工程師遵守)
1. **字體分流策略**：全域主字體統一為 `GoNotoKurrent-Regular.ttf`，備援字體為 `wqy-ZenHei.ttf`。嚴禁擅自引入其他字體導致 JAR 膨脹。
2. **OCR 雙引擎策略**：繁中/簡中/英文優先使用 RapidOCR；其餘語系強制使用 Tesseract。
3. **Tesseract 懶載入與防禦機制**：
   - 語言包路徑固定為 `C:\OCR\tessdata\`。
   - 必須實作 Lazy Loading（按需下載），不將訓練檔打包進 JAR。
   - 實作下載時必須確保：目錄存在（自動建立）、執行緒安全（Thread-Safe 防止併發寫入損毀）、以及優雅的錯誤處理（超時不可卡死主執行緒）。
4. **多頁處理邏輯**：處理 PDF/OFD 時，必須確保迴圈正確遍歷所有頁面。
